...
class TextFieldSearch extends StatefulWidget {
  ...
  final List<SearchModel>? initialList;
...
class _TextFieldSearchState extends State<TextFieldSearch> {
  ...
  void resetState(List<SearchModel> tempList) {
    if (mounted) {
      setState(() {
        // after loop is done, set the filteredList state from the tempList
        filteredList = tempList;
    ...
    return ListView.separated(
      ...
      itemBuilder: (context, i) {
          ...
          return Column(
            children: [
              ListTile(
                onTap: () {
                  // Navigator.push(
                  //     context, PageTransition(child: ProductListingPage.route, (searchQuery: widget.controller.text), type: PageTransitionType.fade));
                  context.push(Uri(path: '/${ProductView.route}', queryParameters: {"search": widget.controller.text}).toString());
                  resetList();
                  FocusScope.of(context).unfocus();
                },
                title: Text('Search for "${widget.controller.text}"', style: AppStyles.getRegularTextStyle(fontSize: 16)),
              ),
              const SizedBox(height: 10),
              ListTile(
                onTap: () {
                  print(filteredList![i].name);
                  context.go('/${ProductView.route}/${filteredList![i].urlKey}.${filteredList![i].urlSuffix}');

                  resetList();
                  FocusScope.of(context).unfocus();
                },
                leading: Image.network(filteredList![i].imgUrl!, height: 50, width: 50),
                title: Text(filteredList![i].name!, style: AppStyles.getRegularTextStyle(fontSize: 16)),
              )
            ],
          );
        }
        return ListTile(
          onTap: () {
            print(filteredList![i].name);
            //TODO  navigate(context, ProductDetails.route, arguments: filteredList![i].sku);
            context.go('/${ProductView.route}/${filteredList![i].urlKey}.${filteredList![i].urlSuffix}');
  ...
  num heightByLength(int length) {
    return (itemHeight * length) + itemHeight + 10;
  }
